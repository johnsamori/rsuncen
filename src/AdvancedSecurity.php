...
class AdvancedSecurity
{
    ...
    public function loadFromStorage(): bool
    {
		...
		// Convert LEGACY_ADMIN to ADMIN
        foreach ($this->UserLevelPrivs as $row) {
            $priv = intval($row[2]);
            if ($priv) {
                if (($priv & Allow::LEGACY_ADMIN->value) == Allow::LEGACY_ADMIN->value) {
                    $priv = Allow::ADMIN->value; // Convert to ADMIN

                    // Update value to database
                    $sql = "UPDATE " . Config("USER_LEVEL_PRIV_TABLE") . " SET " .
                        Config("USER_LEVEL_PRIV_PRIV_FIELD") . " = " . $priv . " WHERE " .
                        Config("USER_LEVEL_PRIV_TABLE_NAME_FIELD") . " = '" . AdjustSql($row[0]) . "' AND " .
                        Config("USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD") . " = " . $row[1];
                    $conn->executeStatement($sql);
                }
            }
        }
