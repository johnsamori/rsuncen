...
class Appointments extends DbTable
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "appointments"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->doctor_id->PleaseSelectText = $this->language->phrase("PleaseSelect"); // "PleaseSelect" text
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
                $this->doctor_id->Lookup = new Lookup($this->doctor_id, 'doctors', false, 'id', ["full_name","","",""], '', "", [], [], [], [], [], [], false, '', '', "`full_name`");
                break;
            default:
                $this->doctor_id->Lookup = new Lookup($this->doctor_id, 'doctors', false, 'id', ["full_name","","",""], '', "", [], [], [], [], [], [], false, '', '', "`full_name`");
                break;
        ...
        // appointment_date
        $this->appointment_date = new DbField(
            $this, // Table
            'x_appointment_date', // Variable name
            'appointment_date', // Name
            '`appointment_date`', // Expression
            CastDateFieldForLike("`appointment_date`", 0, "DB"), // Basic search expression
            135, // Type
            19, // Size
            0, // Date/Time format
            false, // Is upload field
            '`appointment_date`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->appointment_date->InputTextType = "text";
        $this->appointment_date->Raw = true;
        $this->appointment_date->Nullable = false; // NOT NULL field
        $this->appointment_date->Required = true; // Required field
        $this->appointment_date->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $this->language->phrase("IncorrectDate"));
        $this->appointment_date->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['appointment_date'] = &$this->appointment_date;
        ...
        $this->status->addMethod("getDefault", fn() => "scheduled");
        $this->status->InputTextType = "text";
        $this->status->Raw = true;
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
                $this->status->Lookup = new Lookup($this->status, 'appointments', false, '', ["","","",""], '', "", [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->status->Lookup = new Lookup($this->status, 'appointments', false, '', ["","","",""], '', "", [], [], [], [], [], [], false, '', '', "");
                break;
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->appointment_date->DbValue = $row['appointment_date'];
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("appointmentslist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "appointmentsview" => $this->language->phrase("View"),
            "appointmentsedit" => $this->language->phrase("Edit"),
            "appointmentsadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->appointment_date->EditValue = FormatDateTime($this->appointment_date->CurrentValue, $this->appointment_date->formatPattern());
