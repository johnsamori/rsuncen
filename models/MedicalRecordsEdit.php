 ...
 * Page class
 */
class MedicalRecordsEdit extends MedicalRecords
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "MedicalRecordsEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "medicalrecordsedit";
...
// Is modal
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("medicalrecordslist"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("medicalrecordslist"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "medicalrecordslist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "medicalrecordslist") {
                            ...
                            $returnUrl = "medicalrecordslist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'doctor_id' before field var 'x_doctor_id'
        $val = $this->hasFormValue("doctor_id") ? $this->getFormValue("doctor_id") : $this->getFormValue("x_doctor_id");
        ...
        // Check field name 'visit_date' before field var 'x_visit_date'
        $val = $this->hasFormValue("visit_date") ? $this->getFormValue("visit_date") : $this->getFormValue("x_visit_date");
        ...
        // Check field name 'diagnosis' before field var 'x_diagnosis'
        $val = $this->hasFormValue("diagnosis") ? $this->getFormValue("diagnosis") : $this->getFormValue("x_diagnosis");
        ...
        $val = $this->hasFormValue("treatment") ? $this->getFormValue("treatment") : $this->getFormValue("x_treatment");
        ...
        // Check field name 'prescription' before field var 'x_prescription'
        $val = $this->hasFormValue("prescription") ? $this->getFormValue("prescription") : $this->getFormValue("x_prescription");
        if (!$this->prescription->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->prescription->Visible = false; // Disable update for API request
            } else {
                $this->prescription->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->prescription->CurrentValue = $this->prescription->FormValue;
    }
    ...
    public function renderRow(): void
    {
        ...
        // treatment
        $this->treatment->RowCssClass = "row";

        // prescription
        $this->prescription->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->doctor_id->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->doctor_id->Lookup->getTable()->Fields["id"]->searchExpression(), "=", $this->doctor_id->CurrentValue, $this->doctor_id->Lookup->getTable()->Fields["id"]->searchDataType(), "DB");
                }
            ...
            // treatment
            $this->treatment->setupEditAttributes();
            ...
            // prescription
            $this->prescription->setupEditAttributes();
            $this->prescription->EditValue = HtmlEncode($this->prescription->CurrentValue);
            $this->prescription->PlaceHolder = RemoveHtml($this->prescription->caption());
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->doctor_id->Visible && $this->doctor_id->Required) {
                if (!$this->doctor_id->IsDetailKey && IsEmpty($this->doctor_id->FormValue)) {
            ...
            if ($this->visit_date->Visible && $this->visit_date->Required) {
                if (!$this->visit_date->IsDetailKey && IsEmpty($this->visit_date->FormValue)) {
            ...
            if ($this->diagnosis->Visible && $this->diagnosis->Required) {
                if (!$this->diagnosis->IsDetailKey && IsEmpty($this->diagnosis->FormValue)) {
                    $this->diagnosis->addErrorMessage(str_replace("%s", $this->diagnosis->caption(), $this->diagnosis->RequiredErrorMessage));
                }
            }
            if ($this->treatment->Visible && $this->treatment->Required) {
                if (!$this->treatment->IsDetailKey && IsEmpty($this->treatment->FormValue)) {
                    $this->treatment->addErrorMessage(str_replace("%s", $this->treatment->caption(), $this->treatment->RequiredErrorMessage));
                }
            }
            if ($this->prescription->Visible && $this->prescription->Required) {
                if (!$this->prescription->IsDetailKey && IsEmpty($this->prescription->FormValue)) {
                    $this->prescription->addErrorMessage(str_replace("%s", $this->prescription->caption(), $this->prescription->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // patient_id
        $this->patient_id->setDbValueDef($newRow, $this->patient_id->CurrentValue, $this->patient_id->ReadOnly);

        // doctor_id
        $this->doctor_id->setDbValueDef($newRow, $this->doctor_id->CurrentValue, $this->doctor_id->ReadOnly);

        // visit_date
        $this->visit_date->setDbValueDef($newRow, UnFormatDateTime($this->visit_date->CurrentValue, $this->visit_date->formatPattern()), $this->visit_date->ReadOnly);

        // diagnosis
        $this->diagnosis->setDbValueDef($newRow, $this->diagnosis->CurrentValue, $this->diagnosis->ReadOnly);

        // treatment
        $this->treatment->setDbValueDef($newRow, $this->treatment->CurrentValue, $this->treatment->ReadOnly);

        // prescription
        $this->prescription->setDbValueDef($newRow, $this->prescription->CurrentValue, $this->prescription->ReadOnly);
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['diagnosis'])) { // diagnosis
            $this->diagnosis->CurrentValue = $row['diagnosis'];
        }
        if (isset($row['treatment'])) { // treatment
            $this->treatment->CurrentValue = $row['treatment'];
        }
        if (isset($row['prescription'])) { // prescription
            $this->prescription->CurrentValue = $row['prescription'];
        }
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("medicalrecordslist"), "", $this->TableVar, true);
        $pageId = "edit";
