...
class Rooms extends DbTable
{
    ...
    // Fields
    public DbField $id;
    public DbField $room_number;
    public DbField $room_type;
    public DbField $price_per_day;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "rooms"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->room_number->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['room_number'] = &$this->room_number;

        // room_type
        $this->room_type = new DbField(
            $this, // Table
            'x_room_type', // Variable name
            'room_type', // Name
            '`room_type`', // Expression
            '`room_type`', // Basic search expression
            200, // Type
            7, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`room_type`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->room_type->InputTextType = "text";
        $this->room_type->Raw = true;
        $this->room_type->Nullable = false; // NOT NULL field
        $this->room_type->Required = true; // Required field
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
                $this->room_type->Lookup = new Lookup($this->room_type, 'rooms', false, '', ["","","",""], '', "", [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->room_type->Lookup = new Lookup($this->room_type, 'rooms', false, '', ["","","",""], '', "", [], [], [], [], [], [], false, '', '', "");
                break;
        }
        $this->room_type->OptionCount = 3;
        $this->room_type->SearchOperators = ["=", "<>"];
        $this->Fields['room_type'] = &$this->room_type;

        // price_per_day
        $this->price_per_day = new DbField(
            $this, // Table
            'x_price_per_day', // Variable name
            'price_per_day', // Name
            '`price_per_day`', // Expression
            '`price_per_day`', // Basic search expression
            131, // Type
            12, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`price_per_day`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->price_per_day->InputTextType = "text";
        $this->price_per_day->Raw = true;
        $this->price_per_day->Nullable = false; // NOT NULL field
        $this->price_per_day->Required = true; // Required field
        $this->price_per_day->DefaultErrorMessage = $this->language->phrase("IncorrectFloat");
        $this->price_per_day->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['price_per_day'] = &$this->price_per_day;
        ...
        $this->status->addMethod("getDefault", fn() => "available");
        $this->status->InputTextType = "text";
        $this->status->Raw = true;
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
                $this->status->Lookup = new Lookup($this->status, 'rooms', false, '', ["","","",""], '', "", [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->status->Lookup = new Lookup($this->status, 'rooms', false, '', ["","","",""], '', "", [], [], [], [], [], [], false, '', '', "");
                break;
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->price_per_day->DbValue = $row['price_per_day'];
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("roomslist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "roomsview" => $this->language->phrase("View"),
            "roomsedit" => $this->language->phrase("Edit"),
            "roomsadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->price_per_day);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->price_per_day);
