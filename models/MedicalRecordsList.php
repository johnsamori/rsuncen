 ...
 * Page class
 */
class MedicalRecordsList extends MedicalRecords
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "MedicalRecordsList";
    ...
    public string $FormName = "fmedical_recordslist";

    // CSS class/style
    public string $CurrentPageName = "medicalrecordslist";
    ...
    public function setVisibility(): void
    {
        ...
        $this->prescription->setVisibility();
    }
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Table object (medical_records)
        if (!isset($GLOBALS["medical_records"]) || $GLOBALS["medical_records"]::class == PROJECT_NAMESPACE . "medical_records") {
            $GLOBALS["medical_records"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "medicalrecordsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "medicalrecordsdelete";
        $this->MultiUpdateUrl = "medicalrecordsupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'medical_records');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "medicalrecordsview"); // If View page, no primary button
                } else { // List page
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "fmedical_recordsgrid";
        }
		...
		if (@$_COOKIE['medical_records_searchpanel'] == 'notactive' || @$_COOKIE['medical_records_searchpanel'] == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (@$_COOKIE['medical_records_searchpanel'] == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fmedical_recordssrch");
        }
        ...
        $filterList = Concat($filterList, $this->prescription->AdvancedSearch->toJson(), ","); // Field prescription
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fmedical_recordssrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field prescription
        $this->prescription->AdvancedSearch->SearchValue = $filter["x_prescription"] ?? "";
        $this->prescription->AdvancedSearch->SearchOperator = $filter["z_prescription"] ?? "";
        $this->prescription->AdvancedSearch->SearchCondition = $filter["v_prescription"] ?? "";
        $this->prescription->AdvancedSearch->SearchValue2 = $filter["y_prescription"] ?? "";
        $this->prescription->AdvancedSearch->SearchOperator2 = $filter["w_prescription"] ?? "";
        $this->prescription->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        $searchFlds[] = &$this->treatment;
        $searchFlds[] = &$this->prescription;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->prescription); // prescription
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"medical_records\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"medical_records\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"medical_records\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fmedical_recordslist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fmedical_recordslist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"medical_records\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($this->language->phrase("DeleteSelectedLink")) . "\" form=\"fmedical_recordslist\"" .
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "treatment");
            $this->createColumnOption($option, "prescription");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmedical_recordssrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmedical_recordssrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmedical_recordslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_medical_records", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_medical_records",
...
// Load basic search values
    ...
    public function loadRowValues(array|bool|null $row = null): void
    {
        ...
        $this->prescription->setDbValue($row['prescription']);
    }
    ...
    protected function newRow(): array
    {
        ...
        $row['diagnosis'] = $this->diagnosis->DefaultValue;
        $row['treatment'] = $this->treatment->DefaultValue;
        $row['prescription'] = $this->prescription->DefaultValue;
        return $row;
    }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // prescription
            $this->prescription->ViewValue = $this->prescription->CurrentValue;
            ...
            // prescription
            $this->prescription->HrefValue = "";
            $this->prescription->TooltipValue = "";
        }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (@$_COOKIE['medical_records_searchpanel'] == 'notactive' || @$_COOKIE['orders_searchpanel'] == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmedical_recordssrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmedical_recordssrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
