 ...
 * Page class
 */
class HospitalizationEdit extends Hospitalization
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "HospitalizationEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "hospitalizationedit";
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("hospitalizationlist"); // Return to list page
                        return;
...
// Get upload files
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // room_id
        $this->room_id->setDbValueDef($newRow, $this->room_id->CurrentValue, $this->room_id->ReadOnly);

        // admission_date
        $this->admission_date->setDbValueDef($newRow, UnFormatDateTime($this->admission_date->CurrentValue, $this->admission_date->formatPattern()), $this->admission_date->ReadOnly);

        // discharge_date
        $this->discharge_date->setDbValueDef($newRow, UnFormatDateTime($this->discharge_date->CurrentValue, $this->discharge_date->formatPattern()), $this->discharge_date->ReadOnly);

        // total_cost
        $this->total_cost->setDbValueDef($newRow, $this->total_cost->CurrentValue, $this->total_cost->ReadOnly);
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['room_id'])) { // room_id
            $this->room_id->CurrentValue = $row['room_id'];
        }
        if (isset($row['admission_date'])) { // admission_date
            $this->admission_date->CurrentValue = $row['admission_date'];
        }
        if (isset($row['discharge_date'])) { // discharge_date
            $this->discharge_date->CurrentValue = $row['discharge_date'];
        }
