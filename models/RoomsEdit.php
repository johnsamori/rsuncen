...
class RoomsEdit extends Rooms
{
    ...
    // Page object name
    public string $PageObjName = "RoomsEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "roomsedit";
...
// Is modal
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("roomslist"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("roomslist"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "roomslist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "roomslist") {
                            ...
                            $returnUrl = "roomslist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'room_number' before field var 'x_room_number'
        $val = $this->hasFormValue("room_number") ? $this->getFormValue("room_number") : $this->getFormValue("x_room_number");
        ...
        // Check field name 'room_type' before field var 'x_room_type'
        $val = $this->hasFormValue("room_type") ? $this->getFormValue("room_type") : $this->getFormValue("x_room_type");
        if (!$this->room_type->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->room_type->Visible = false; // Disable update for API request
            } else {
                $this->room_type->setFormValue($val);
            }
        }

        // Check field name 'price_per_day' before field var 'x_price_per_day'
        $val = $this->hasFormValue("price_per_day") ? $this->getFormValue("price_per_day") : $this->getFormValue("x_price_per_day");
        if (!$this->price_per_day->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->price_per_day->Visible = false; // Disable update for API request
            } else {
    ...
    public function renderRow(): void
    {
        ...
        // room_type
        $this->room_type->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // room_type
            $this->room_type->EditValue = $this->room_type->options(false);
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->room_number->Visible && $this->room_number->Required) {
                if (!$this->room_number->IsDetailKey && IsEmpty($this->room_number->FormValue)) {
                    $this->room_number->addErrorMessage(str_replace("%s", $this->room_number->caption(), $this->room_number->RequiredErrorMessage));
                }
            }
            if ($this->room_type->Visible && $this->room_type->Required) {
                if ($this->room_type->FormValue == "") {
                    $this->room_type->addErrorMessage(str_replace("%s", $this->room_type->caption(), $this->room_type->RequiredErrorMessage));
                }
            }
            if ($this->price_per_day->Visible && $this->price_per_day->Required) {
    ...
    protected function editRow(): bool
    {
        ...
        // Check field with unique index (room_number)
        if ($this->room_number->CurrentValue != "") {
            $filterChk = "(`room_number` = '" . AdjustSql($this->room_number->CurrentValue) . "')";
            ...
            if ($rsChk->fetchAssociative()) {
                $idxErrMsg = sprintf($this->language->phrase("DuplicateIndex"), $this->room_number->CurrentValue, $this->room_number->caption());
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // room_number
        $this->room_number->setDbValueDef($newRow, $this->room_number->CurrentValue, $this->room_number->ReadOnly);

        // room_type
        $this->room_type->setDbValueDef($newRow, $this->room_type->CurrentValue, $this->room_type->ReadOnly);

        // price_per_day
        $this->price_per_day->setDbValueDef($newRow, $this->price_per_day->CurrentValue, $this->price_per_day->ReadOnly);
    ...
    {
        if (isset($row['room_number'])) { // room_number
            $this->room_number->CurrentValue = $row['room_number'];
        }
        if (isset($row['room_type'])) { // room_type
            $this->room_type->CurrentValue = $row['room_type'];
        }
        if (isset($row['price_per_day'])) { // price_per_day
            $this->price_per_day->CurrentValue = $row['price_per_day'];
        }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("roomslist"), "", $this->TableVar, true);
        $pageId = "edit";
