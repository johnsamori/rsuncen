...
class RoomsList extends Rooms
{
    ...
    // Page object name
    public string $PageObjName = "RoomsList";
    ...
    public string $FormName = "froomslist";

    // CSS class/style
    public string $CurrentPageName = "roomslist";
...
// Is modal
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->room_type);
		...
		if (@$_COOKIE['rooms_searchpanel'] == 'notactive' || @$_COOKIE['rooms_searchpanel'] == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (@$_COOKIE['rooms_searchpanel'] == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("froomssrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->room_number->AdvancedSearch->toJson(), ","); // Field room_number
        $filterList = Concat($filterList, $this->room_type->AdvancedSearch->toJson(), ","); // Field room_type
        $filterList = Concat($filterList, $this->price_per_day->AdvancedSearch->toJson(), ","); // Field price_per_day
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field price_per_day
        $this->price_per_day->AdvancedSearch->SearchValue = $filter["x_price_per_day"] ?? "";
        $this->price_per_day->AdvancedSearch->SearchOperator = $filter["z_price_per_day"] ?? "";
        $this->price_per_day->AdvancedSearch->SearchCondition = $filter["v_price_per_day"] ?? "";
        $this->price_per_day->AdvancedSearch->SearchValue2 = $filter["y_price_per_day"] ?? "";
        $this->price_per_day->AdvancedSearch->SearchOperator2 = $filter["w_price_per_day"] ?? "";
        $this->price_per_day->AdvancedSearch->save();
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->price_per_day); // price_per_day
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"rooms\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"rooms\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"rooms\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"froomslist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"froomslist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"rooms\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($this->language->phrase("DeleteSelectedLink")) . "\" form=\"froomslist\"" .
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "room_number");
            $this->createColumnOption($option, "room_type");
            $this->createColumnOption($option, "price_per_day");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"froomssrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"froomssrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
...
// Load basic search values
    ...
    protected function newRow(): array
    {
        ...
        $row['room_type'] = $this->room_type->DefaultValue;
        $row['price_per_day'] = $this->price_per_day->DefaultValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // room_type
            if (strval($this->room_type->CurrentValue) != "") {
                $this->room_type->ViewValue = $this->room_type->optionCaption($this->room_type->CurrentValue);
            } else {
                $this->room_type->ViewValue = null;
            }
            ...
            $this->price_per_day->TooltipValue = "";
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (@$_COOKIE['rooms_searchpanel'] == 'notactive' || @$_COOKIE['orders_searchpanel'] == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"froomssrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"froomssrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
