...
<form name="fhospitalizationadd" id="fhospitalizationadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_hospitalization_patient_id">
    <select
        ...
        data-select2-id="fhospitalizationadd_x_patient_id"
        <?php } ?>
...
<?php if (!$Page->patient_id->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fhospitalizationadd", function() {
    var options = { name: "x_patient_id", selectId: "fhospitalizationadd_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fhospitalizationadd.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fhospitalizationadd" };
    } else {
        options.ajax = { id: "x_patient_id", form: "fhospitalizationadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_hospitalization_room_id">
    <select
        ...
        data-select2-id="fhospitalizationadd_x_room_id"
        <?php } ?>
        data-table="hospitalization"
...
<script<?= Nonce() ?>>
loadjs.ready("fhospitalizationadd", function() {
    var options = { name: "x_room_id", selectId: "fhospitalizationadd_x_room_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fhospitalizationadd.lists.room_id?.lookupOptions.length) {
        options.data = { id: "x_room_id", form: "fhospitalizationadd" };
    } else {
        options.ajax = { id: "x_room_id", form: "fhospitalizationadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script<?= Nonce() ?>>
loadjs.ready(["fhospitalizationadd", "datetimepicker"], function () {
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(['fhospitalizationadd', 'jqueryinputmask'], function() {
	options = {
		'jitMasking': false,
		'removeMaskOnSubmit': true
	};
	ew.createjQueryInputMask("fhospitalizationadd", "x_admission_date", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
loadjs.ready(['fhospitalizationadd', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fhospitalizationadd", "x_discharge_date", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
loadjs.ready(['fhospitalizationadd', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fhospitalizationadd", "x_total_cost", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fhospitalizationadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fhospitalizationadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fhospitalizationadd:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
