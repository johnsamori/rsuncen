...
<form name="fmedical_recordsadd" id="fmedical_recordsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_medical_records_patient_id">
    <select
        ...
        data-select2-id="fmedical_recordsadd_x_patient_id"
        <?php } ?>
...
<?php if (!$Page->patient_id->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fmedical_recordsadd", function() {
    var options = { name: "x_patient_id", selectId: "fmedical_recordsadd_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmedical_recordsadd.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fmedical_recordsadd" };
    } else {
        options.ajax = { id: "x_patient_id", form: "fmedical_recordsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_medical_records_doctor_id">
    <select
        ...
        data-select2-id="fmedical_recordsadd_x_doctor_id"
        <?php } ?>
        data-table="medical_records"
...
<script<?= Nonce() ?>>
loadjs.ready("fmedical_recordsadd", function() {
    var options = { name: "x_doctor_id", selectId: "fmedical_recordsadd_x_doctor_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmedical_recordsadd.lists.doctor_id?.lookupOptions.length) {
        options.data = { id: "x_doctor_id", form: "fmedical_recordsadd" };
    } else {
        options.ajax = { id: "x_doctor_id", form: "fmedical_recordsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
...
<script<?= Nonce() ?>>
loadjs.ready(["fmedical_recordsadd", "datetimepicker"], function () {
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(['fmedical_recordsadd', 'jqueryinputmask'], function() {
	options = {
		'jitMasking': false,
		'removeMaskOnSubmit': true
	};
	ew.createjQueryInputMask("fmedical_recordsadd", "x_visit_date", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fmedical_recordsadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fmedical_recordsadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fmedical_recordsadd:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
