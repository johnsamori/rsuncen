...
<span id="el_appointments_doctor_id">
    <select
        ...
        data-select2-id="fappointmentsedit_x_doctor_id"
        <?php } ?>
        data-table="appointments"
...
<script<?= Nonce() ?>>
loadjs.ready("fappointmentsedit", function() {
    var options = { name: "x_doctor_id", selectId: "fappointmentsedit_x_doctor_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fappointmentsedit.lists.doctor_id?.lookupOptions.length) {
        options.data = { id: "x_doctor_id", form: "fappointmentsedit" };
    } else {
        options.ajax = { id: "x_doctor_id", form: "fappointmentsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(['fappointmentsedit', 'jqueryinputmask'], function() {
	options = {
		'jitMasking': false,
		'removeMaskOnSubmit': true
	};
	ew.createjQueryInputMask("fappointmentsedit", "x_appointment_date", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('#btn-action').on('click',function(){if(fappointmentsedit.validateFields()){ew.prompt({title: ew.language.phrase("MessageEditConfirm"),icon:'question',showCancelButton:true},result=>{if(result)$("#fappointmentsedit").submit();});return false;} else { ew.prompt({title: ew.language.phrase("MessageInvalidForm"), icon: 'warning', showCancelButton:false}); }});});
</script>
<?php } ?>
<script<?= Nonce() ?>>
...
<script>
loadjs.ready("head", function() { $("#fappointmentsedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
